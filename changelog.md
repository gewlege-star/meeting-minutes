...
## [0.3.0-beta] - 2026-05-30

### Added

- **Glossary 智慧提示注入 (Glossary prompt injection to Whisper)**: 將詞彙表設定的專有名詞自動附加到 OpenAI 相容模型（如 Groq whisper、OpenAI whisper）轉錄 API 的 Prompt 中，提升 Whisper 對自訂特定字串、人名、專業術語的原生辨識準確度，防止後處理替換失效。
- 並且，更新 `ask-km.md` 問答對齊，記錄兩段式架構之整合複雜度與 AI Settings 規劃設計。

## [0.2.0-beta] - 2026-05-30

### Added

- **AI 智能逐字稿矯正功能 (AI Transcript Correction)**: 整合 AI 矯正（"AI 矯正" 🪄 按鈕），配置 OpenAI 與 Gemini 的 Summary 模型對全篇逐字稿進行錯別字、同音異義字、語音辨識模糊與語意通順度的智慧修復，並防禦性保留原始時間戳對齊結構。
- **詞彙表後處理整合 (Glossary Post-processing)**: 在 AI 矯正後自動重新套用自訂詞彙表（Glossary），確保專有名詞與特定術語的絕對準確一致。
...
- **開發知識庫 [ask-km.md](ask-km.md)**: 建立 [ask-km.md](ask-km.md) 文件，記錄本專案開發過程中的重要問答知識（低信心標記原理、說話者識別方案、音檔切割最佳長度、版號邏輯等）。
